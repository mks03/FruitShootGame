...
COPYRIGHT:		(C)2010,2011,2012,2013 MIKE DOGAN GAMES & ENTERTAINMENT
...
local RemoveFXListener = function (name)
	...
	if V.FXFields[name].Listener ~= nil then 
...
local StartEmitter = function (name, oneShotMode)
	...
	for j,pName in ipairs(V.Emitters[name].PTypeIndex) do
		...
		PSettings.startTime       = V.Emitters[name].startTime + PSettings.delay
		PSettings.endTime         = PSettings.startTime + PSettings.duration
...
local SetEmitterSound = function (name, SoundID, delay, autoStop, SoundProperties)
	...
	if V.Emitters[name] ~= nil then 
		...
		-- ATTACH NEW SOUND
		if SoundID ~= nil then
			if SoundProperties == nil then SoundProperties = { channel = 1, loops = 0, fadeIn = 1000 } end
			V.Emitters[name].Sound 		= nil
			V.Emitters[name].SoundProperties= nil
...
local SetEmitterTarget = function (emitterName, FollowTarget, followRotation, followRotationOffset, xOffset, yOffset)
	...
	Emitter.followXOff           = xOffset              ~= nil and xOffset or 0
	Emitter.followYOff           = yOffset              ~= nil and yOffset or 0
end
...
local PrepareParticleSettings = function (name)
	...
	if type(V.ParticleTypes[name].imagePath) ~= "table" then 
		V.ParticleTypes[name].imagePath = { V.ParticleTypes[name].imagePath }
	end
...
local CreateParticleType = function (name, Properties)
	...
	V.ParticleTypes[name].ImageSheet            = nil
	V.ParticleTypes[name].SheetInfo             = nil
	V.ParticleTypes[name].AnimationInfo         = nil
	V.ParticleTypes[name].frames                = nil
	...
	V.ParticleTypes[name].faceEmitter		    = nil
	...
	V.ParticleTypes[name].sound	                = nil
	V.ParticleTypes[name].soundProps            = {channel = 0, loops = 0}
...
local DeleteParticleType = function (name)
	...
	if V.ParticleTypes[name] ~= nil then
		...
		for i,v in pairs(V.ParticleTypes[name]) do V.ParticleTypes[name][i] = nil end
...
local DeleteParticle = function (index)
	...
	if V.Particles[index] ~= nil then
		...
		if V.Particles[index].PType.TouchListener ~= nil then
			V.Particles[index]:removeEventListener("touch", V.Particles[index].PType.TouchListener)
		end
...
local Update = function ()
	...
	local Emitter
	local now       = system.getTimer() - V.gLostTime
	local diffTime  = (now - V.gLastTime) * V.timeScale; V.gLastTime = now; if diffTime == 0 then diffTime = 1 end
	...
	for i,eName in ipairs(V.EmitterIndex) do
		...
		if Emitter.active then
			...
			if Emitter.FollowObject ~= nil then
				emitterX, emitterY   = Emitter.FollowObject.parent:localToContent(Emitter.FollowObject.x + Emitter.followXOff, Emitter.FollowObject.y + Emitter.followYOff)
				emitterX, emitterY   = EmitterParent:contentToLocal(emitterX, emitterY)
				Emitter.x, Emitter.y = emitterX, emitterY
			...
			for j,pName in ipairs(Emitter.PTypeIndex) do
				...
				if PSettings.active == true and now > PSettings.startTime then
					...
					while PSettings.particlesToEmit >= 1.0 do
						...
						-- NEW PARTICLE
						----------------------------------------
						-- TAKE FRAME FROM IMAGE SHEET TEXTURE (NEW SPRITE API)
						if PType.ImageSheet ~= nil then
							if PType.AnimationInfo == nil then
								local frame       = PType.frames[ V.Rnd(1,#PType.frames) ]
								V.Particles[slot] = display.newImageRect(PType.ImageSheet, frame, PType.SheetInfo.frames[frame].width, PType.SheetInfo.frames[frame].height)
							else
								V.Particles[slot] = display.newSprite(PType.ImageSheet, PType.AnimationInfo)
								V.Particles[slot]:play()
							end
							V.Particles[slot].xReference = PType.xReference
							V.Particles[slot].yReference = PType.yReference
						-- TEXT PARTICLE
						elseif PType.text ~= nil then
						...
						elseif PType.SpriteSet == nil then
							V.Particles[slot] = display.newImageRect(PType.imagePath[ V.Rnd(1,#PType.imagePath) ], PType.imageWidth,PType.imageHeight)
							V.Particles[slot].xReference = PType.xReference
							V.Particles[slot].yReference = PType.yReference
						-- ANIMATED
						else
						...
						-- PLAY PARTICLE SOUND?
						if PType.sounds ~= nil then
							if type(PType.sounds) == "table" then
								audio.play(PType.sounds[V.Rnd(#PType.sounds)], PType.soundProps)
							else
								audio.play(PType.sounds, PType.soundProps)
							end
						end
						...
						-- ADD TAP / TOUCH LISTENER?
						if PType.TapListener   ~= nil then Particle:addEventListener("tap", PType.TapListener) end
						if PType.TouchListener ~= nil then Particle:addEventListener("touch", PType.TouchListener) end
						...
						if PType.faceEmitter == nil then      
							...
							dirAngle			= emitterRot
						elseif PType.faceEmitter == false then 
